...
class BostonFeaturesTransformer(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, degree=2):
        ...
        # ====== YOUR CODE: ======
        self.cust_features = PolynomialFeatures(degree, interaction_only=True, include_bias=False)
        # ========================
...
def top_correlated_features(df: DataFrame, target_feature, n=5):
    ...
    for i in df.columns[:-1]:
        ...
        sig_x = (((df[i] - df[i].mean()) ** 2).sum()) ** .5
        sig_y = (((y - y.mean()) ** 2).sum()) ** .5
        corr = sig_xy / (sig_x * sig_y)
        top_n_corr = np.append(top_n_corr, corr)
    res = np.argsort(abs(top_n_corr))[::-1][:n]
    top_n_corr = top_n_corr[res]
    top_n_features = df.columns[res]
    # ========================
...
def cv_best_hyperparams(
    ...
    search = sklearn.model_selection.GridSearchCV(estimator=model,
