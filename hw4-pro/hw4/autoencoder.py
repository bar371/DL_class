...
class DecoderCNN(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # ====== YOUR CODE: ======
        modules.extend(
            (nn.ConvTranspose2d(in_channels=in_channels, out_channels=out_channels, kernel_size=5, stride=2, padding=2),
            nn.BatchNorm2d(out_channels, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True),
            nn.ReLU(),
            nn.ConvTranspose2d(in_channels=out_channels, out_channels=out_channels, kernel_size=3, padding=1),
            nn.BatchNorm2d(out_channels, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True),
            nn.ReLU(),
            nn.ConvTranspose2d(in_channels=out_channels, out_channels=out_channels, kernel_size=5, padding=2),
            nn.BatchNorm2d(out_channels, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True),
            nn.ReLU(),
            nn.ConvTranspose2d(in_channels=out_channels, out_channels=out_channels, kernel_size=4, padding=1))
        )
        # ========================
        self.cnn = nn.Sequential(*modules)
...
class VAE(nn.Module):
    def __init__(self, features_encoder, features_decoder, in_size, z_dim):
        ...
        self.z_decode = nn.Linear(in_features=z_dim, out_features=n_features)
        # ========================
    ...
    def encode(self, x):
        ...
        mu = self.mu(h)
        log_sigma2 = self.sigma(h)
        # Reparametrization trick: Sample u from an isotropic gaussian and
        # use it to create z.
        reparam = torch.randn(mu.size(), device=mu.device)
        z = mu + reparam * log_sigma2
        # ========================
    ...
    def decode(self, z):
        ...
        # ====== YOUR CODE: ======
        # step 1
        h = self.z_decode(z).reshape(z.shape[0], *self.features_shape)
        # step 2 
